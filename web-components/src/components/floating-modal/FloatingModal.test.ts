...
describe("Floating Modal Component", () => {
  ...
  test("should seed the minimized chip from minPosition after a normal-modal drag, not jump from zero", async () => {
    const element = await fixture<FloatingModal.ELEMENT>(html`
      <md-floating-modal
        .show=${true}
        .minimizable=${true}
        .position=${{ x: 200, y: 100 }}
        .minPosition=${{ x: 600, y: 400 }}
      >
  ...
  test("should seed the normal modal from position after a minimized-chip drag, not jump from zero", async () => {
    const element = await fixture<FloatingModal.ELEMENT>(html`
      <md-floating-modal
        .show=${true}
        .minimizable=${true}
    ...
    const restoreButton = minModal.shadowRoot!.querySelector(".md-floating__resize") as Button.ELEMENT;
    restoreButton.shadowRoot!.querySelector("button")!.click();
    await elementUpdated(element);
