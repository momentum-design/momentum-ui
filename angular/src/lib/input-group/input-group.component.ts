...
@Component({
  ...
  template: `
    ...
    <!-- Secondary Label -->
    <label class="md-input__secondary-label" [htmlFor]='_control.id' *ngIf="secondaryLabel">
      <span>{{ secondaryLabel }}</span>
    </label>

    <!-- Helper Text -->
    <md-input-helper *ngIf="inputHelpText" [id]='_inputHelpTextId'>
      {{inputHelpText}}
    </md-input-helper>
...
export class InputGroupComponent implements AfterContentInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  public messages: Array<[]>;
   ...
   _inputHelpTextId: string = `md-input__help-text-${nextUniqueId++}`;
  ...
  ngAfterContentInit() {
    ...
    this._contentMessageChildren.changes.pipe(startWith(null)).subscribe(() => {
      this._syncDescribedByIds();
      this._changeDetectorRef.markForCheck();
    });

    // Update the padding class when icon children change.
    this._iconChildren.changes.pipe(startWith(null)).subscribe(() => {
      this._syncInputPadding();
      this._changeDetectorRef.markForCheck();
    });
  }
    ...
    if (this._control) {
      const ids: string[] = [];
        ...
        if (this.inputHelpText) {
          ids.push(this._inputHelpTextId);
