...
const setComponentData = (id, component) => {
  return { type: types.FETCH_DATAVISUALIZATION_SUCCESS, id, component };
};

const setError = error => {
  return { type: types.FETCH_DATAVISUALIZATION_ERROR, error };
};
...
export const fetchComponentData = id => dispatch => {
  return fetch(`${config.DATA_VISUALIZATION_URL}/${id}`)
    .then(res => res.json())
