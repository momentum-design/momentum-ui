...
@Component({
  ...
  template: `
    ...
    <span class="cui-slider__selection" [ngStyle]="{'left.%': selectionLeft, 'width.%': selectionWidth}"></span>
    <cui-slider-pointer
      *ngIf="isObject(value)"
    ...
    <div #ticksContainer>
      <span
        *ngFor="let tickValue of scale"
        class="cui-slider__hashlabel"
      >{{ translateFn && translateFn(tickValue) || tickValue }}</span>
    </div>
  `,
  host: {
    'class': 'cui-slider',
...
export class SliderComponent implements AfterViewInit, OnInit, ControlValueAccessor {
  ...
  @Input()
  get value(): SliderChange | number { return this._value; }
  set value(value) {
    if (this._value !== value) {
      this._value = value;
      this.sliderLow = isObject(this.value) && (this.value as SliderChange).low || this.min;
      this.sliderHigh = isObject(this.value) && (this.value as SliderChange).high || this.value as number;
      this.returnCurrentValues();
      this.changeDetectorRef.markForCheck();
    }
  }
  /** Event emitted when change a pointer position */
  @Output() readonly change: EventEmitter<SliderChange | number> = new EventEmitter<SliderChange | number>();
  ...
  get sliderLow(): number { return this._sliderLow; }
  set sliderLow(value) { this._sliderLow = value; }
  get sliderHigh(): number { return this._sliderHigh; }
  set sliderHigh(value) { this._sliderHigh = value; }
  ...
  setScalePos(scale: number[], min: number, max: number) {
    this.ticksContainer.nativeElement.querySelectorAll('span').forEach((child: HTMLElement, idx: number) => {
  ...
  moveForward(key: string, pixelMove: number, limit: number) {
    const newPosition = this[key] + pixelMove <= limit
      ? this[key] + pixelMove
      : limit;
  ...
  getSelectionWidth() {
    ...
    this.selectionLeft = ((Math.min(baseValue, this.sliderHigh) - this.min) / this.max) * 100;
    this.selectionWidth = (Math.abs(this.sliderHigh - baseValue) / this.max) * 100;
  }
}
