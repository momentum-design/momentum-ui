...
export class DatePickerSandbox extends LitElement {
  @property({ type: Boolean }) cancelButtonDisabled = false;
  @property({ type: Boolean }) acceptButtonDisabled = false;

  render() {
    return html` <h2 class="sandbox-header">md-datepicker</h2>
      <h3>row calendar</h3>
      ...
      <md-datepicker
        .controlButtons=${{
          apply: { value: "Apply", disabled: this.acceptButtonDisabled },
          cancel: { value: "Cancel", disabled: this.cancelButtonDisabled }
        }}
