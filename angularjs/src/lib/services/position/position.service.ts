 ...
 */
$position.$inject = ['$document', '$window'];
export function $position($document, $window) {
  /**
  ...
  return {
    ...
    scrollParent: function (elem, includeHidden) {
      ...
      let overflowRegex = includeHidden ? OVERFLOW_REGEX.hidden : OVERFLOW_REGEX.normal;
      ...
      while (scrollParent.parentElement && scrollParent !== documentEl) {
        let spStyle = $window.getComputedStyle(scrollParent);
    ...
    viewportOffset: function (elem, useDocument, includePadding) {
      ...
      let offsetParent = useDocument ? $document[0].documentElement : this.scrollParent(elem);
    ...
    positionElements: function (hostElem, targetElem, placement, appendToBody) {
      ...
      if (placement[2]) {
        ...
        let targetElemStyle = $window.getComputedStyle(targetElem);
        let adjustedSize = {
      ...
      if (targetElementLeftMargin) {
        targetElemPos.left += targetElementLeftMargin;
      }
    ...
    */
    positionArrow: function (elem, placement) {
      ...
      let innerElem = elem.querySelector('.tooltip-inner, .popover-inner, .c-nub');
      if (!innerElem) {
        return;
      }

      let isTooltip = angular.element(innerElem).hasClass('c-nub');

      let arrowElem = isTooltip ? elem.querySelector('.nub') : elem.querySelector('.arrow');
      if (!arrowElem) {
        return;
      }
      ...
      let borderWidth = $window.getComputedStyle(arrowElem)[borderProp];
