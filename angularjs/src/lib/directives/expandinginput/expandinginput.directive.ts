...
export function csExpandinginputCtrl() {
  let vm = this;
  vm.elemIsInFocus = false;
}

csExpandinginput.$inject = ['$timeout'];
export function csExpandinginput($timeout) {
  let directive = {
    scope: {
      model: '=ngModel',
      focusWidth: '=',
    ...
    restrict: 'A',
    controller: csExpandinginputCtrl,
    controllerAs: 'csExpandinginput',
    ...
    link: function postLink(scope, element, attrs) {
      ...
      element.css('minWidth', attrs.csExpandinginputMinwidth || element.css('minWidth'));
      element.css('maxWidth', attrs.csExpandinginputMaxwidth || defaultMaxwidth);
      let elemIsInFocus;
